 ...
 *
 * Wraps libsodium's XChaCha20-Poly1305 (IETF) AEAD construction: 256-bit keys,
 * 192-bit random nonces (wide enough to generate randomly without a counter),
 * and a Poly1305 tag that authenticates both the ciphertext and the associated
 * data. The associated data binds a ciphertext to its context (subject set,
 * KEK id, …) so it cannot be silently relocated to another record.
 *
 * Output blobs are `nonce || ciphertext+tag` as raw bytes; callers base64 them
